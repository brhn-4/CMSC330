...
def clean_input(string)
	clean_pattern = /(\'|;|\<|\>|\-\-)/
	string = string.gsub(clean_pattern){|match| ""}
	string
end


def check_and_authorize_user(user,session,token)
	if (authorize(user,session) && @tokens[user] == token) then
		true
	else
		false
	end
...
module Access
	...
	def authenticate(user, passwd)
		assign_session user if (get_user(user))&& get_password(user) == passwd
	end
...
module User
	...
	def search(user_query)
			...
			FROM Users
			WHERE User LIKE '%#{cleaned_query}%' OR
			Description LIKE '%#{cleaned_query}%'
			}
			@db.execute(query) do |user|
					users << {
							:name => user[0],
			...
			{ :query => cleaned_query, :users => users }
	end
	...
	def register(user, filename, file, password, confirm)
		if ((password == confirm) && (get_username(user) == nil)) then
	...
	def update_prefs(user, session, description, token)
		...
		cleaned_description = clean_input(description)
		if (check_and_authorize_user(user,session,token))
		query = %{
		UPDATE Users
